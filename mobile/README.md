...
4. (Tuỳ chọn) **Kiểm tra kết nối** — nút gọi `GET /info` (danh tính) + `GET /provision` (cấu hình đã
   lưu — SSID, peerMac) để xác nhận điện thoại tới được ESP. Lưu ý: trình duyệt **không** đọc được tên
   WiFi / IP của điện thoại (giới hạn bảo mật web), nên phần chẩn đoán dựa vào 2 endpoint này của ESP.
