...
Trên PWA:
1. Bấm **Nhập tay** (hoặc dán QR mẫu mà mock-server in ra log — JSON `{id,role,ap,ip,mac}`).
2. Bật **Chế độ Test** (POST sẽ trỏ tới `http://localhost:8080`).
3. Điền SSID/WiFi + (role main) email/password thiết bị + peerMac + hset/deadband.
4. Bấm **Gửi cấu hình** → thấy **response** `{ok:true, message:"Saved. Rebooting.", mac}` hiển thị
   trên màn hình; cửa sổ console mock-server log lại các field nhận được.
...
### B2. Provisioning từng board (lần đầu boot)

Mỗi ESP chưa cấu hình sẽ phát SoftAP `PROV-SENSOR-xxxxxx` / `PROV-MAIN-xxxxxx` và in **QR JSON** ra
Serial.

1. **ESP2 (sensor):** PWA quét QR ESP2 → nối AP `PROV-SENSOR-...` → POST `ssid` (các field khác có thể
   để trống ở Basic vì ESP2 không dùng WiFi). Ghi lại **MAC ESP2** (có trong QR/Serial).
2. **ESP1 (main):** PWA quét QR ESP1 → nối AP `PROV-MAIN-...` → POST `ssid`, `password`,
   `deviceEmail`, `devicePassword`, **`peerMac` = MAC ESP2**, `hset`, `deadband`.
