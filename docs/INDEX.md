...
- ✅ **App gọn**: chỉ WiFi + MAC, có **2 checkbox (WiFi/MAC) gửi từng phần** + nút **test `GET /info` + `/provision`** (chẩn đoán, gọi riêng từng endpoint) + trạng thái online. Firmware `POST /provision` hỗ trợ **partial update** (nhận cả `peerMac`/`mac_gateway`).
